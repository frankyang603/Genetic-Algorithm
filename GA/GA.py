...
#CrossoverType:PartialMappedCrossover 
...
class GeneticAlgorithm:
    def __init__(self,pop_size,number_of_genes,selection_type,crossover_rate,mutation_rate,compute_objective_value):
        ...
        self.mapping = [-1 for i in range(self.number_of_genes)] 
...
with open('input.json', 'r') as inputFile:
    ...
    for key in data:
        ...
        jap = JAPProblem(input)
        jap.compute_objective_value(range(len(input)))

        pop_size = jap.number_of_jobs*jap.number_of_jobs*jap.number_of_jobs
        selection_type = SelectionType(2)
        crossover_rate = 0.2
        mutation_rate = 0.1
        solver = GeneticAlgorithm(pop_size,jap.number_of_jobs,selection_type,crossover_rate,
            mutation_rate,jap.compute_objective_value)

        solver.initialize()

        for i in range(jap.number_of_jobs*10):
        ...
        print(f"{key}\nAssignment: {solver.best_chromosome.tolist()}\nCost: {jap.compute_objective_value(solver.best_chromosome)}\n")
